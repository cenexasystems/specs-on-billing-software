...
export default function CatalogModal({ isOpen, onClose, onAdd }: CatalogModalProps) {
  ...
  const [lensTypes, setLensTypes] = useState<{ id: number; name: string }[]>([])
  ...
  useEffect(() => {
    ...
    void loadCategories()
    supabase.from('lens_types').select('id, name').eq('is_active', true).order('sort_order')
      .then(({ data }) => setLensTypes((data || []) as { id: number; name: string }[]))
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl sm:rounded-3xl w-full max-w-5xl flex min-h-0 flex-col shadow-2xl overflow-hidden border border-[#e4d6c5]/40 max-h-[calc(100dvh-1rem)] sm:max-h-[85vh]">

        {editingProduct ? (
          <>
            <div className="flex items-center justify-between p-6 border-b border-[#e4d6c5]/40 bg-[#fbf8f2]">
            ...
            <form onSubmit={saveEdit} className="p-6 flex flex-col gap-4">
              ...
              <div>
                ...
                <input type="text" value={editForm.name}
                  ...
                  className="w-full px-4 py-3 bg-[#fbf8f2] border border-[#e4d6c5]/60 rounded-xl focus:outline-none focus:border-[#5a3928] text-[13px] font-bold" />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editForm.category}
                    ...
                    className="w-full min-w-0 h-12 px-4 py-3 bg-[#fbf8f2] border border-[#e4d6c5]/60 rounded-xl focus:outline-none focus:border-[#5a3928] text-[13px] font-bold touch-manipulation">
                ...
                <div>
                  ...
                  <input type="number" value={editForm.price}
                    ...
                    className="w-full px-4 py-3 bg-[#fbf8f2] border border-[#e4d6c5]/60 rounded-xl focus:outline-none focus:border-[#5a3928] text-[13px] font-bold text-right" placeholder="0" />
                </div>
              </div>
              <button type="submit" disabled={editLoading}
                className="mt-4 w-full py-3.5 bg-[#5a3928] hover:bg-[#065F46] text-white rounded-xl text-[13px] font-black uppercase tracking-wider transition-colors disabled:opacity-50">
          ...
          <>
            <div className="flex items-center justify-between p-5 border-b border-[#e4d6c5]/40 bg-[#fbf8f2]">
              <h2 className="text-[18px] font-black text-[#111111] flex items-center gap-2">
                <Search size={18} className="text-[#5a3928]" />
                Search Catalog
              </h2>
            ...
            <div className="p-3 sm:p-4 border-b border-[#e4d6c5]/40 bg-white space-y-3">
              <div className="relative">
                ...
                <input type="text" value={search}
                  ...
                  className="w-full pl-10 pr-4 py-3 bg-[#FAFAFA] border border-[#e4d6c5]/60 rounded-xl focus:outline-none focus:border-[#5a3928] text-[13px] font-bold text-[#111111]" />
              </div>
              <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                {categories.map(cat => (
                  <button key={cat} onClick={() => setActiveCategory(cat)}
                    className={`px-4 py-2 rounded-xl text-[11px] font-black uppercase tracking-wider whitespace-nowrap transition-colors ${activeCategory === cat ? 'bg-[#5a3928] text-white' : 'bg-[#FAFAFA] text-[#374151] hover:bg-[#fbf8f2] border border-[#e4d6c5]/60'}`}>
                    {cat}
                  </button>
                ))}
              </div>
            ...
            <div className="min-h-0 flex-1 overflow-y-auto p-3 sm:p-4 bg-[#FAFAFA]">
              {loading ? (
                <div className="flex min-h-48 flex-col items-center justify-center gap-3 text-[#374151]/70">
                  <span className="h-7 w-7 animate-spin rounded-full border-2 border-[#e4d6c5] border-t-[#5a3928]" />
                ...
                <div className="flex min-h-48 flex-col items-center justify-center gap-2 px-4 text-center text-red-500">
                  ...
                  <button type="button" onClick={() => void fetchProducts(true)} className="rounded-lg bg-[#5a3928] px-3 py-2 text-[11px] font-black text-white">Try again</button>
                </div>
              ) : filtered.length === 0 ? (
                ...
                <div className="grid grid-cols-1 min-[360px]:grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                  {filtered.map(product => (
                    <div key={product.id}
                      className="bg-white border border-[#e4d6c5]/60 rounded-2xl p-3 flex flex-col gap-2 hover:border-[#5a3928]/40 hover:shadow-md transition-all group relative">
                      <div onClick={() => onAdd(product)} className="cursor-pointer flex-1">
                        <div className="flex items-start justify-between gap-2">
                          <div className="min-w-0 flex-1">
                            <h4 className="text-[13px] font-black text-[#111111] leading-tight group-hover:text-[#5a3928] transition-colors break-words">{product.name}</h4>
                            {product.nameTa && <p className="text-[10px] font-bold text-[#374151] mt-0.5 truncate">{product.nameTa}</p>}
                            {product.lensTypeId && <p className="text-[10px] font-black uppercase tracking-wide text-[#5a3928] mt-1 truncate">{lensTypes.find(type => String(type.id) === String(product.lensTypeId))?.name || 'Lens'}</p>}
                          </div>
                          <div className="flex gap-1 shrink-0" onClick={e => e.stopPropagation()}>
                            <button onClick={(e) => { e.stopPropagation(); startEdit(product) }} title="Edit product"
                              className="p-1.5 rounded-lg bg-white border border-[#e4d6c5]/60 text-[#374151] hover:text-[#5a3928] hover:border-[#5a3928]/40 shadow-sm transition-colors">
                              <Edit2 size={14} />
                            </button>
                            <button onClick={(e) => { e.stopPropagation(); void handleDelete(product) }} title="Delete product"
                              className="p-1.5 rounded-lg bg-white border border-[#e4d6c5]/60 text-red-400 hover:text-red-600 hover:border-red-300 shadow-sm transition-colors">
                              <Trash2 size={14} />
                            </button>
                          </div>
                        </div>
                      ...
                      <div onClick={() => onAdd(product)} className="cursor-pointer">
                        <div className="flex items-end justify-between mt-2 pt-2 border-t border-[#e4d6c5]/30">
                          <span className="text-[14px] font-black text-[#111111]">{product.pricingType === 'manual' ? 'Manual price' : `₹${product.price}`}</span>
                          <span className="text-[9px] font-black text-[#374151] uppercase tracking-wider bg-[#fbf8f2] px-2 py-1 rounded border border-[#e4d6c5]/40 truncate max-w-[50%]">{product.category}</span>
                        </div>
                      </div>
                    </div>
                  ))}
