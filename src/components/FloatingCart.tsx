    ...
    <AnimatePresence>
      {itemCount > 0 && !isCartOrCheckout && (
        ...
        >
          <Link
            to={user ? '/cart' : '/login'}
            className="pointer-events-auto flex items-center gap-2.5 rounded-full border border-white/10 bg-[#2f211b]/95 px-4 py-2 text-white shadow-[0_8px_28px_rgba(30,47,28,0.45)] backdrop-blur-xl transition-transform hover:scale-[1.02] active:scale-[0.98] sm:gap-3 sm:px-5 sm:py-2.5"
          >
            {/* Icon + badge */}
            <div className="relative shrink-0">
              ...
              <span className="absolute -right-1 -top-1 flex h-4 w-4 items-center justify-center rounded-full bg-[#8a5b3d] text-[9px] font-black text-white">
                {itemCount}
              </span>
            </div>
