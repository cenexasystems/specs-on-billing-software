...
export const createOrderWithStock = async (input: CreateOrderInput): Promise<CreatedOrder> => {
  ...
  // Match the canonical 19-parameter RPC deployed in Supabase.
  ...
  const rpcResult = await supabase.rpc('create_order_with_stock', rpcPayload)
  data = rpcResult.data
  ...
  if (error) {
    if (typeof error === 'object' && error !== null && 'message' in error) {
      ...
      const context = [err.details, err.hint, err.code]
        .filter(value => value !== undefined && value !== null && String(value).trim())
        .map(value => String(value))
        .join(' | ')
      console.error('[orderService] create_order_with_stock failed', {
        message,
        details: err.details,
        hint: err.hint,
        code: err.code,
  ...
  const phoneParts = normalizeIndianPhone(phone)
  if (phoneParts) {
    const { error: phonePartsError } = await supabase.from('orders').update({
      phone_country_code: phoneParts.countryCode,
      phone_number: phoneParts.phoneNumber,
      phone: phoneParts.phoneNumber,
    }).eq('id', orderId)
    if (phonePartsError) console.warn('[orderService] phone parts sync failed:', phonePartsError.message)
  }
