...
const getUsers = (): LocalUser[] => {
  try {
    const raw = localStorage.getItem('generic_business_users') || '[]'
...
const saveUsers = (u: LocalUser[]) => localStorage.setItem('generic_business_users', JSON.stringify(u))
...
export const authService = {
  ...
  signUp: async (params: {
    ...
    if (isSupabaseConfigured) {
      ...
      return {
        user: {
          ...
          mobile: normalizePhone(String(profile?.mobile_number || profile?.mobile || data.user.user_metadata?.mobile || params.mobile)) || '',
    ...
    localStorage.setItem('generic_business_session', newUser.id)
  ...
  signIn: async (email: string, password: string): Promise<{ user: AuthUser | null; error: string | null }> => {
    ...
    if (isSupabaseConfigured) {
      ...
      return {
        user: {
          ...
          mobile: normalizePhone(String(profile?.mobile_number || profile?.mobile || data.user.user_metadata?.mobile || '')) || '',
    ...
    localStorage.setItem('generic_business_session', match.id)
  ...
  signOut: async (): Promise<void> => {
    ...
    if (USE_LOCAL_AUTH_FALLBACK) {
      localStorage.removeItem('generic_business_session')
    }
  },
  ...
  getCurrentUser: async (): Promise<AuthUser | null> => {
    if (isSupabaseConfigured) {
      ...
      return {
        ...
        mobile: normalizePhone(String(profile?.mobile_number || profile?.mobile || user.user_metadata?.mobile || '')) || '',
        email: user.email || '',
        role: profile?.role === 'admin' ? 'admin' : 'customer',
      }
    }
    ...
    // localStorage fallback
    const sid = localStorage.getItem('generic_business_session')
    if (!sid) return null
    const users = getUsers()
      ...
      const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
        if (session?.user) {
          ...
          callback({
            ...
            mobile: normalizePhone(String(profile?.mobile_number || profile?.mobile || session.user.user_metadata?.mobile || '')) || '',
