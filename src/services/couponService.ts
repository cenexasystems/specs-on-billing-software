...
export async function validateCoupon(
  ...
  try {
    ...
    return {
      data: { code: String(row.code), percentage: Number(row.percentage || 0), discountType, discountValue, maximumDiscount: row.maximum_discount == null ? null : Number(row.maximum_discount), discount: Number(row.discount || 0) },
      error: null,
    }
  } catch (error) {
