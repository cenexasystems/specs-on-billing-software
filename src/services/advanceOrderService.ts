...
import { formatPhoneDisplay, normalizePhone, phonePartsFromRecord } from '../lib/phone'
...
const STORAGE_ORDERS_KEY = 'generic_business_advance_orders_v1'
const STORAGE_TIMELINE_KEY = 'generic_business_advance_timeline_v1'
const STORAGE_PAYMENTS_KEY = 'generic_business_advance_payments_v1'
...
export async function createAdvanceOrder(input: {
  ...
  if (isSupabaseConfigured) {
    const { data, error } = await supabase.rpc('create_advance_order', {
      p_customer_name: input.customerName, p_phone: normalizePhone(input.phone) || '', p_address: input.address, p_product_name: input.productName,
    ...
    if (data) {
      ...
      const parts = normalizePhone(createdOrder.phone)
      if (parts) {
        await supabase.from('advance_orders').update({ phone_country_code: '+91', phone_number: parts, phone: parts }).eq('id', createdOrder.id)
      }
  ...
  if (!createdOrder) {
    ...
    const depositId = `PB-ADV-${ymd}-${seq}`
...
export async function editAdvanceOrder(orderId: string, input: {
  customerName: string; phone: string; address: string; productName: string; category: string; description: string
  totalAmount: number; expectedDeliveryDate: string; remarks: string; referenceNumber: string; products?: Array<Record<string, unknown>>
}): Promise<AdvanceOrder> {
  if (!isSupabaseConfigured) throw new Error('Supabase is required to edit advance orders')
  const { data, error } = await supabase.rpc('update_advance_order', {
    p_order_id: orderId, p_customer_name: input.customerName, p_phone: normalizePhone(input.phone) || input.phone,
    p_address: input.address, p_product_name: input.productName, p_category: input.category, p_description: input.description,
    p_total_amount: input.totalAmount, p_expected_delivery_date: input.expectedDeliveryDate, p_remarks: input.remarks,
    p_reference_number: input.referenceNumber, p_products: input.products || [],
  })
  if (error) throw new Error(error.message || 'Unable to edit advance order')
