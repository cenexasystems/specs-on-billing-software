...
import { isValidPhone, normalizeIndianPhone, normalizePhone } from '../lib/phone'
...
const STATUS_COLORS: Record<string, { bg: string; text: string; label: string }> = {
  ...
  completed: { bg: '#e4d6c5', text: '#065F46', label: 'Completed' },
...
export default function Profile() {
  ...
  const handleSaveProfile = async () => {
    ...
    const phoneParts = normalizeIndianPhone(editPhone)
    const trimPhone = phoneParts?.phoneNumber || ''
    ...
    const { error } = await supabase
      .from('profiles')
      .update({ name: trimName, mobile: trimPhone, mobile_country_code: phoneParts?.countryCode || null, mobile_number: phoneParts?.phoneNumber || null })
      .eq('id', user.id)
  ...
  useEffect(() => {
    ...
    const loadOrders = async () => {
      try {
        ...
        const phoneDigits = normalizePhone(user.mobile || '') || ''
                ...
                {editing && (
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="block text-[11px] font-bold text-textMuted uppercase tracking-wide mb-1.5">
                        Mobile Number
                        <span className="ml-1 font-normal normal-case text-[10px] text-gray-400">Include country code</span>
                      </label>
                      <div className="flex gap-2">
                        ...
                        <input
                          ...
                          value={editPhone}
                          onChange={e => { setEditPhone(e.target.value); setSaveErr('') }}
                          placeholder="0123456789"
                        />
                      </div>
                    </div>

                    <div className="p-3 bg-[#fbf8f2] rounded-xl text-[12px] text-textMuted">
