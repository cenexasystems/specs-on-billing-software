...
import { invoicePdfFileWithCanonicalLogo } from '../lib/invoicePdf'
...
import { advanceReceiptPdf, downloadFile, printAdvanceReceipt, printPdfFile } from '../lib/advanceReceipt'
...
const currencyIcon = ({ size = 20, className = '' }: { size?: number; className?: string }) => (
...
export default function AdvanceOrders({ onOrderCompleted }: AdvanceOrdersProps = {}) {
  ...
  const create = async (event: FormEvent) => {
    ...
    if (!Number.isFinite(total) || total <= 0 || !Number.isFinite(deposit) || deposit <= 0 || deposit >= total) { setError(`Deposit must be greater than ${formatCurrency(0)} and less than the total order amount.`); setSaving(false); return }
    if (!normalizePhone(form.phone.trim())) { setError('Please enter a valid Indian mobile number (e.g. 9876543210 or +91 9876543210).'); setSaving(false); return }
    try {
      const created = await createAdvanceOrder({ ...form, phone: normalizePhone(form.phone) || '', totalAmount: total, depositAmount: deposit, referenceNumber: form.reference_number, createdByName: role || 'Staff', products: [{ name: form.productName, category: form.category, description: form.description, quantity: 1, base_price: total, line_total: total, unit: 'piece', unit_type: 'unit', source: 'advance_order' }] })
  ...
  const invoiceFile = (order: AdvanceOrder) => invoicePdfFileWithCanonicalLogo({ invoiceNo: order.invoice_number || order.deposit_id, date: order.completed_at || new Date().toISOString(), customerName: order.customer_name, phone: order.phone, address: order.address, items: productRows(order), subtotal: order.total_amount, shipping: 0, total: order.total_amount, paymentMode: order.final_payment_method || 'Paid', advancePaid: order.deposit_amount, finalPayment: Math.max(0, order.total_amount - order.deposit_amount) })
  const printFinal = (order: AdvanceOrder) => void printPdfFile(invoiceFile(order))
  ...
  const cards = [
    ...
    ['Total Deposit Amount', formatCurrency(analytics.deposits), currencyIcon, 'text-fuchsia-700 bg-fuchsia-50'], ['Outstanding Balance', formatCurrency(analytics.outstanding), currencyIcon, 'text-red-700 bg-red-50'],
