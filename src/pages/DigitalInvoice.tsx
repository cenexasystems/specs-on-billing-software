...
const invoiceItems = (Array.isArray(invoice.items) ? invoice.items : [])
  ...
  const shareViaWhatsApp = async () => {
    const invoiceElement = invoiceElementRef.current
    ...
    const file = await invoicePdfFileFromElement(invoiceElement, invoice.invoice_no)
...
<div className="flex items-center gap-2">
          <button
            onClick={downloadPdf}
            className="flex items-center gap-2 bg-[#5a3928] text-white px-5 py-2 rounded-full font-bold text-sm shadow-md hover:bg-[#2f211b] transition-colors"
          >
            <Printer size={16} /> PDF
          </button>
          <button
