...
export default function ProductDetails() {
  ...
  if (loading) return <div className="min-h-screen bg-[#fbfaf6] p-10 text-center font-bold text-[#3a271d]">Loading...</div>
    ...
    <div className="min-h-screen bg-[#fbfaf6] pb-[calc(7.75rem+env(safe-area-inset-bottom))]">
      <div className="lg:hidden sticky top-0 z-30 border-b border-white/60 bg-[#fbfaf6]/92 px-4 py-3 backdrop-blur sm:px-6">
        <div className="mx-auto flex max-w-3xl items-center justify-between gap-3">
          <Link
            to="/products"
            className="inline-flex items-center gap-2 rounded-full border border-[#dfcdb7]/60 bg-white px-3 py-2 text-[11px] font-black text-[#3a271d] shadow-sm"
          >
      ...
      <div className="lg:hidden mx-auto flex max-w-3xl flex-col gap-0 px-0 sm:px-6 lg:px-8">
        ...
        <section className="px-4 pt-4 sm:px-0 sm:pt-5">
          <div className="space-y-2">
            ...
            <h1 className="text-[1.55rem] leading-tight font-black text-[#3a271d] sm:text-4xl">{displayName}</h1>
            {product.nameTa && <p className="text-base font-bold text-[#5f6d59] ta-text sm:text-lg">{product.nameTa}</p>}

            <div className="flex flex-wrap items-center gap-2 pt-1">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-[11px] font-black text-[#3a271d] shadow-sm ring-1 ring-[#dfcdb7]/50">
                <Star size={12} className="fill-amber-400 text-amber-400" />
                {(product.rating || 4.7).toFixed(1)}
              </span>
              <span className="inline-flex items-center gap-2 rounded-full bg-[#f7f4ed] px-3 py-1.5 text-[11px] font-black text-[#5f6d59] shadow-sm ring-1 ring-[#dfcdb7]/45">
        ...
        <section className="px-4 pt-4 sm:px-0">
          <div className="rounded-[24px] bg-white/95 p-3.5 shadow-sm ring-1 ring-[#dfcdb7]/55 sm:p-4">
            <AnimatePresence mode="wait">
              {mobileQty === 0 ? (
                <motion.button
                  ...
                  type="button"
                  className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#3a271d] py-3 text-sm font-black text-white shadow-[0_16px_30px_rgba(44,57,42,0.22)]"
                >
                ...
                >
                  ...
                  <div className="inline-flex w-full items-center justify-between gap-2 rounded-full bg-white px-2 py-1 shadow-sm ring-1 ring-[#dfcdb7]/55">
                    <button
                      type="button"
      ...
      <div className="hidden lg:block mx-auto flex max-w-3xl flex-col gap-0 px-0 sm:px-6 lg:px-8">
        ...
        <section className="px-4 pt-4 sm:px-0 sm:pt-5">
          <div className="space-y-2">
            <p className="text-[10px] font-black uppercase tracking-[0.22em] text-[#7daa8f]">{t('cat.' + product.category)}</p>
            <h1 className="text-[1.8rem] leading-tight font-black text-[#3a271d] sm:text-4xl">{displayName}</h1>
            {product.nameTa && <p className="text-base font-bold text-[#5f6d59] ta-text sm:text-lg">{product.nameTa}</p>}
          ...
          <div className="rounded-[24px] bg-white/95 p-3.5 shadow-sm ring-1 ring-[#dfcdb7]/55 sm:p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="text-right">
                ...
                <p className="text-lg font-black text-[#3a271d]">{formatCurrency(lineTotal)}</p>
              </div>
            </div>
        ...
        <section className="px-4 pt-4 sm:px-0">
          <div className="grid gap-2.5">
            <details className={accordionClass}>
              ...
              <p className="mt-3 text-sm leading-relaxed text-[#5f6d59]">{displayDesc || 'Carefully selected product product made for daily use.'}</p>
            </details>
          ...
          <div className="mt-3 flex gap-3 overflow-x-auto pb-2 hide-scrollbar">
            ...
            {relatedProducts.map((item) => (
              <button
                ...
                className="group min-w-[124px] overflow-hidden rounded-[20px] bg-white text-left shadow-sm ring-1 ring-[#dfcdb7]/55 transition-transform hover:-translate-y-0.5"
              >
                ...
                <div className="space-y-1.5 p-2.5">
                  <p className="line-clamp-2 text-[11px] font-bold leading-snug text-[#3a271d]">{item.name}</p>
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-[#dfcdb7]/50 bg-white/95 px-4 py-3 backdrop-blur pb-[calc(env(safe-area-inset-bottom)+0.75rem)]">
        <div className="mx-auto flex max-w-3xl items-center gap-3">
          <div className="min-w-0">
            <p className="text-[11px] font-bold text-[#7daa8f]">Total</p>
            <p className="text-base font-black leading-tight text-[#3a271d]">{formatCurrency(lineTotal)}</p>
          </div>
          <button
            onClick={handleAdd}
            className="flex-1 rounded-2xl bg-[#3a271d] py-3.5 text-sm font-black text-white shadow-[0_16px_30px_rgba(44,57,42,0.28)]"
            type="button"
          >
