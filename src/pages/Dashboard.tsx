...
import { formatPhoneDisplay, normalizeIndianPhone, normalizePhone, phonePartsFromRecord } from '../lib/phone'
import {
...
const CurrencyIcon = ({ size = 16, className = '' }: { size?: number; className?: string }) => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
...
import { invoicePdfFileFromElement, invoicePdfFileWithCanonicalLogo } from '../lib/invoicePdf'
...
import CatalogPricingPanel from '../components/dashboard/CatalogPricingPanel'
...
type DashboardOrder = {
  id: string; invoice_no: string; customer_name: string; phone: string; phone_country_code: string; phone_number: string; address: string
...
type TabKey = 'overview' | 'whatsapp' | 'pos_analytics' | 'billing' | 'advance_orders' | 'products' | 'catalog_pricing' | 'categories' | 'coupons' | 'users' | 'history'
...
const emptyForm = {
  ...
  hasVariants: false,
  pricingType: 'fixed' as 'fixed' | 'manual', productType: 'standard' as 'standard' | 'lens' | 'other', brandType: 'none' as 'none' | 'non_branded' | 'branded',
}

const exportXlsx = (orders: DashboardOrder[]) => {
  const header = ['Order Ref', 'Customer', 'Country Code', 'Mobile Number', 'Date', 'Total (₹)', 'Order Type', 'Status']
  const rows = orders.map(o => {
    const phone = normalizeIndianPhone(o.phone)
    return [
    o.order_type === 'online_request' ? o.id : o.invoice_no, o.customer_name,
    phone?.countryCode || '+91', phone?.phoneNumber || String(o.phone || ''),
    new Date(o.created_at).toLocaleDateString('en-MY'),
    getOrderTotal(o).toFixed(2), o.order_type, o.status,
    ]
  })
  downloadXlsx(`orders_${new Date().toISOString().slice(0, 10)}.xlsx`, 'Orders', header, rows, [2, 3])
}
...
const UNIT_TYPE_OPTIONS: { value: UnitType; label: string; hint: string }[] = [
  ...
  { value: 'bundle', label: 'Bundle / Set',    hint: 'e.g. Pooja kit, Product pack' },
]
...
export default function Dashboard() {
  ...
  const [couponForm, setCouponForm] = useState({ code: '', discount_type: 'percentage' as 'percentage' | 'fixed', discount_value: '10', percentage: 10, maximum_discount: '', expiry_date: '', usage_limit: '', min_order_value: '' })
  ...
  // Every authenticated workspace user may manage the business data. RLS is
  // configured with the same authenticated-user rule in the billing migration.
  const isAdmin = true
  ...
  const toDashboardOrder = (row: Record<string, unknown>): DashboardOrder => ({
    ...
    phone: formatPhoneDisplay(phonePartsFromRecord(row)?.phoneNumber || String(row.phone || '')),
    phone_country_code: phonePartsFromRecord(row)?.countryCode || '+91',
    phone_number: phonePartsFromRecord(row)?.phoneNumber || String(row.phone || ''),
  ...
  const _handleAdvanceOrderCompleted = useCallback((advance: AdvanceOrder) => {
    ...
    const completed: DashboardOrder = {
      ...
      phone_number: normalizePhone(advance.phone) || '',
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const [cRes, initialOrdersRes, couponRes] = await Promise.all([
        supabase.from('categories').select('id, name_en, name_ta, is_active, sort_order').order('sort_order'),
        supabase.from('orders')
          .select('id, invoice_no, customer_name, phone, phone_country_code, phone_number, address, created_at, total, status, order_mode, order_type, user_id, items, coupon_code, discount_amount, manual_discount_amount, delivery_charge, total_gst, gst_amount, payment_mode, payment_method, invoice_pdf_url, remarks, reference_number')
      ...
      // Older Supabase projects may not have the optional metadata columns yet.
      // Do not discard all history/analytics in that case; retry with the core
      // order shape and keep the optional fields empty.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let oRes: { data: unknown[] | null; error: any } = {
        data: initialOrdersRes.data as unknown[] | null,
        error: initialOrdersRes.error,
      }
      if (oRes.error) {
        console.warn('[Dashboard] Retrying orders query with core columns:', oRes.error.message)
        const fallbackOrdersRes = await supabase.from('orders')
          .select('id, invoice_no, customer_name, phone, address, created_at, total, status, order_mode, order_type, user_id, items, coupon_code, discount_amount, manual_discount_amount, delivery_charge, total_gst, gst_amount, payment_mode, payment_method')
          .order('created_at', { ascending: false })
          .limit(1000)
        oRes = {
          data: fallbackOrdersRes.data as unknown[] | null,
          error: fallbackOrdersRes.error,
        }
      }
  ...
  const handlePrintReceipt = (_order: DashboardOrder) => window.print()
  ...
  const openOrderInvoice = async (order: DashboardOrder, mode: 'view' | 'download' | 'print') => {
    ...
    // When the preview is open, capture that exact rendered component. This
    // prevents an older stored PDF from diverging from what the user sees.
    if (mode === 'download' && invoicePreviewRef.current && invoicePreviewOrder?.id === order.id) {
      const file = await invoicePdfFileFromElement(invoicePreviewRef.current, order.invoice_no || order.id)
    ...
    const file = invoicePreviewRef.current
      ? await invoicePdfFileFromElement(invoicePreviewRef.current, order.invoice_no || order.id)
      : await invoicePdfFileWithCanonicalLogo({
  ...
  const saveCoupon = async (e: FormEvent) => {
    ...
    const discountValue = toNumber(couponForm.discount_value, 0)
    if (!(discountValue > 0) || (couponForm.discount_type === 'percentage' && discountValue > 100)) {
      setCouponSaveError(couponForm.discount_type === 'percentage' ? 'Discount must be between 1% and 100%' : 'Fixed discount must be greater than 0'); return
    }
    const code = couponForm.code.trim().toUpperCase()
    const payload = {
      code,
      percentage: couponForm.discount_type === 'percentage' ? discountValue : 0,
      discount_type: couponForm.discount_type,
      discount_value: discountValue,
      maximum_discount: couponForm.maximum_discount ? toNumber(couponForm.maximum_discount, 0) : null,
      is_active: true,
    ...
    if (error) {
      ...
      if (msg.toLowerCase().includes('row-level security')) {
        setCouponSaveError('Coupon could not be saved. Apply migration 0008_allow_authenticated_workspace_management.sql, then refresh the page.')
      } else {
        setCouponSaveError(msg.includes('unique') || msg.includes('duplicate') ? `Coupon code "${code}" already exists` : msg)
      }
    } else {
      setCouponForm({ code: '', discount_type: 'percentage', discount_value: '10', percentage: 10, maximum_discount: '', expiry_date: '', usage_limit: '', min_order_value: '' })
  ...
  const startEditCoupon = (coupon: DashboardCoupon) => {
    ...
    setCouponForm({
      ...
      discount_value: String(coupon.discount_value || coupon.percentage || 0),
  ...
  const runSearch = async (e?: FormEvent) => {
    ...
    try {
      ...
      const initialSearchResult = await q
      let data: unknown[] | null = initialSearchResult.data as unknown[] | null
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      let error: any = initialSearchResult.error
      if (error) {
        console.warn('[Dashboard] Retrying order search with core columns:', error.message)
        let fallback = supabase.from('orders')
          ...
          .order('created_at', { ascending: false })
          .limit(500)
        if (search.invoiceNo.trim())       fallback = fallback.ilike('invoice_no', `%${search.invoiceNo.trim()}%`)
        if (search.phone.trim())           fallback = fallback.ilike('phone', `%${search.phone.trim()}%`)
        if (search.customerName.trim())    fallback = fallback.ilike('customer_name', `%${search.customerName.trim()}%`)
        if (search.dateFrom)               fallback = fallback.gte('created_at', `${search.dateFrom}T00:00:00`)
        if (search.dateTo)                 fallback = fallback.lte('created_at', `${search.dateTo}T23:59:59`)
        if (billTypeFilter === 'manual')   fallback = fallback.eq('order_type', 'manual_sale')
        else if (billTypeFilter === 'offline') fallback = fallback.eq('order_type', 'pos_sale').eq('order_mode', 'offline')
        else if (billTypeFilter === 'online')  fallback = fallback.eq('order_type', 'pos_sale').eq('order_mode', 'online')
  ...
  const handleSaveProd = async (e: FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        pricing_type: prodForm.pricingType,
        product_type: prodForm.productType,
        brand_type: prodForm.brandType,
      ...
      // Do not coerce the write response with .single(). PostgREST may return
      // an empty representation when RLS/returning visibility is involved,
      // even though the write itself is valid.
      const result = editingProd
  ...
  const handleEdit = (p: Product) => {
    ...
    setProdForm({
      ...
      pricingType: p.pricingType ?? 'fixed', productType: p.productType ?? 'standard', brandType: p.brandType ?? 'none',
    } as typeof prodForm)
    setVariantNotice('')
  ...
  if (!isAdmin) return (
    <div className="min-h-screen bg-bgMain flex items-center justify-center p-4">
      <div className="bg-white p-8 rounded-3xl shadow-xl text-center max-w-sm">
        ...
        <h2 className="text-2xl font-black mb-2">{l('Unauthorized', '?…?©? மதி இல? லை')}</h2>
        <Link to="/" className="px-6 py-3 bg-sageDark text-white rounded-xl font-bold inline-block mt-4">{l('Go Home', 'ம? ?•ப? பிற? ?•? ')}</Link>
      </div>
    </div>
  )
  ...
  const allNavItems: Array<{ id: TabKey; icon: React.ReactNode; label: string }> = [
    ...
    { id: 'catalog_pricing', icon: <CurrencyIcon size={20} />,    label: 'Pricing & Lenses' },
  ...
  return (
    <div className="admin-shell h-screen min-h-screen bg-bgMain flex flex-col lg:flex-row overflow-hidden">
      ...
      <main className="flex-grow flex flex-col overflow-hidden">
        ...
        {tab === 'overview' && (() => {
          ...
          <div className="space-y-6 rounded-[28px] bg-maroon-dark p-5 sm:p-6 lg:p-7 shadow-2xl border border-white/10 text-white">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-xl font-black text-[#111111]">{l('Analytics Dashboard', 'ப?•ுப்பாய்வு தட்டு')}</h2>
              <div className="flex items-center gap-2">
                <button onClick={() => void loadData()}
                  className="flex items-center gap-1.5 px-3 py-2 bg-white border border-[#e4d6c5]/40 rounded-xl text-[12px] font-bold text-[#374151] hover:bg-[#fbf8f2]">
                  <RefreshCw size={13} /> {l('Refresh', 'புதுப்பி')}
                </button>
              </div>
            </div>

            <div className="flex gap-2 overflow-x-auto pb-2 border-b border-[#e4d6c5]/40">
              {[
                ...
                <button key={sub.id} onClick={() => setAnalyticsTab(sub.id)}
                  className={`px-4 py-2 rounded-xl text-[13px] font-bold whitespace-nowrap transition-colors ${analyticsTab === sub.id ? 'bg-[#111111] text-white' : 'bg-white border border-[#e4d6c5]/40 text-[#374151] hover:bg-[#fbf8f2]'}`}>
                  {sub.label}
                </button>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-3">
              {[
                { label: l('Total Revenue', 'மொத்த வருவாய்'),    value: formatCurrency(analytics.totalCompletedRevenue), from: 'from-emerald-50 via-emerald-50/80 to-teal-50', iconBg: 'from-emerald-400 to-teal-500', icon: <CurrencyIcon size={16} /> },
                { label: l("Today's Sales",  'இ?©்றைய விற்ப?©ை'),  value: formatCurrency(analytics.todaySales),            from: 'from-blue-50 via-blue-50/80 to-indigo-50', iconBg: 'from-blue-400 to-indigo-500', icon: <TrendingUp size={16} /> },
                { label: l('Offline Revenue', 'ஆஃப்லை?©் வருவாய்'), value: formatCurrency(analytics.posRevenue),           from: 'from-orange-50 via-orange-50/80 to-amber-50', iconBg: 'from-orange-400 to-amber-500', icon: <CurrencyIcon size={16} /> },
                { label: l('Online Revenue',  'ஆ?©்லை?©் வருவாய்'),  value: formatCurrency(analytics.onlinePosRevenue),     from: 'from-cyan-50 via-cyan-50/80 to-sky-50', iconBg: 'from-cyan-400 to-sky-500', icon: <CurrencyIcon size={16} /> },
                { label: l('Manual Revenue',  '?•ைமுறை வருவாய்'),   value: formatCurrency(analytics.manualRevenue),        from: 'from-violet-50 via-violet-50/80 to-purple-50', iconBg: 'from-violet-400 to-purple-500', icon: <ShoppingCart size={16} /> },
              ].map((card, i) => (
            ...
            {/* Latest POS Bills */}
            <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-base font-black text-[#111111]">{l('Latest POS Bills', 'POS பில்?•ள்')}</h3>
                <button onClick={() => setTab('billing')} className="text-[12px] font-bold text-[#8a5b3d] hover:underline">{l('View All →', '?…?©ைத்தும் →')}</button>
              </div>
              {latestPOS.length > 0 ? (
                <div className="overflow-x-auto rounded-xl border border-[#e4d6c5]/30">
                  <table className="w-full min-w-[480px] text-[12px]">
                    <thead className="bg-[#fbf8f2] text-[10px] uppercase tracking-wider text-[#374151]">
                      <tr>
                        <th className="px-3 py-2.5 font-black text-left">{l('Invoice', 'பில்')}</th>
                        <th className="px-3 py-2.5 font-black text-left">{l('Customer', 'வாடி?•்?•ையாளர்')}</th>
                        ...
                        <th className="px-3 py-2.5 font-black text-left">{l('Type', 'வ?•ை')}</th>
                        <th className="px-3 py-2.5 font-black text-left">{l('Status', 'நிலை')}</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-[#e4d6c5]/20">
                      {latestPOS.map(o => {
                        ...
                        return (
                          <tr key={o.id} className="hover:bg-[#fbf8f2]/50">
                            <td className="px-3 py-2.5 font-bold text-[#8a5b3d] text-[11px]">{o.invoice_no || '-'}</td>
                ...
                <p className="text-[13px] text-[#374151] text-center py-4">{l('No bills yet', 'பில்?•ள் இல்லை')}</p>
              )}
            </div>
            </>
            )}
                ...
                <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                  ...
                  <div className="space-y-3">
                    {analytics.topProducts.slice(0, 10).map((p, i) => (
                      <div key={i} className="flex justify-between items-center bg-[#fbf8f2] p-3 rounded-xl">
                        <div>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-[11px] font-bold text-[#8a5b3d]">{formatCurrency(p.revenue)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div className="bg-[#fbf8f2] p-4 rounded-xl flex justify-between items-center">
                    ...
                    <div className="bg-[#fbf8f2] p-4 rounded-xl flex justify-between items-center">
                      ...
                      <span className="text-[16px] font-black text-[#8a5b3d]">
                  ...
                  <div className="space-y-3">
                    {analytics.topCoupons.slice(0, 5).map((c, i) => (
                      <div key={i} className="flex justify-between items-center bg-[#fbf8f2] p-3 rounded-xl">
                        ...
                        <span className="text-[12px] font-bold text-[#8a5b3d]">{c.usage} uses</span>
                      </div>
                    ))}
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <h2 className="text-xl font-black text-[#111111]">{l('WhatsApp Center', 'வாட்ஸ் ?…ப் மையம்')}</h2>
                ...
                <div className="flex gap-1">
                  {(['all', 'today', 'week', 'month'] as const).map(preset => (
                    <button key={preset} type="button" onClick={() => applyAnalyticsPreset(preset)}
                      className={`px-2.5 py-1.5 rounded-lg text-[11px] font-black transition-colors ${analyticsDatePreset === preset ? 'bg-[#111111] text-white' : 'bg-[#fbf8f2] text-[#374151] hover:bg-[#e4d6c5]/40'}`}>
                      {preset === 'all' ? l('All','எல்லாம்') : preset === 'today' ? l('Today','இ?©்று') : preset === 'week' ? l('Week','வாரம்') : l('Month','மாதம்')}
                    </button>
                  ))}
                </div>
                <button onClick={() => void loadData()}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-[#e4d6c5]/40 rounded-lg text-[11px] font-bold text-[#374151] hover:bg-[#fbf8f2]">
            ...
            <div className="grid grid-cols-4 gap-3">
              {[
                { label: l('Total Requests', 'மொத்த ?•ோரி?•்?•ை'), val: analytics.waRequests,  bg: 'bg-blue-50',   color: 'text-blue-700',   border: 'border-blue-100' },
            ...
            <div className="bg-white rounded-2xl border border-blue-200 shadow-sm">
              <div className="flex items-center justify-between px-5 py-4 border-b border-blue-100">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="text-base font-black text-[#111111]">{l('Customer Requests', 'வாடி?•்?•ையாளர் ?•ோரி?•்?•ை?•ள்')}</h3>
                  <span className="text-[10px] font-bold text-[#9BAB9A] bg-[#fbf8f2] px-2 py-0.5 rounded-full">{l('₹0 revenue - status updates only', '₹0 வருவாய் - நிலை மட்டும்')}</span>
                </div>
                <span className="text-[12px] text-[#374151] font-bold">{analytics.onlineRequestOrders.length} {l('requests', '?•ோரி?•்?•ை?•ள்')}</span>
              </div>
              ...
              {analytics.onlineRequestOrders.length > 0 ? (
                  ...
                  <table className="w-full text-[12px] min-w-[820px]">
                    <thead className="bg-blue-50 border-b border-blue-100 sticky top-0 z-10">
                      <tr className="text-left text-[#374151] font-black text-[10px] uppercase tracking-wider">
                        <th className="px-4 py-3">{l('Customer', 'வாடி?•்?•ையாளர்')}</th>
                        <th className="px-4 py-3">{l('Phone', 'தொலைபேசி')}</th>
                        <th className="px-4 py-3">{l('Address', 'மு?•வரி')}</th>
                        <th className="px-4 py-3 text-center">{l('Products', 'பொருட்?•ள்')}</th>
                                ...
                                <td colSpan={8} className="px-4 pb-5 pt-2 bg-blue-50/40">
                                  <div className="space-y-4">
                                    ...
                                    <div className="flex flex-wrap gap-4 text-[12px] bg-white rounded-xl p-3 border border-blue-100">
                                      ...
                                      <div className="flex-1"><span className="font-black text-[#374151]">{l('Address', 'மு?•வரி')}: </span><span className="text-[#111111]">{order.address || '-'}</span></div>
                                        ...
                                        <table className="w-full text-[12px] min-w-[540px] bg-white rounded-xl overflow-hidden border border-blue-100">
                                          <thead className="bg-[#fbf8f2]">
                                            <tr className="text-left text-[#374151] font-black text-[10px] uppercase tracking-wider">
                                              ...
                                              <th className="px-4 py-2.5">{l('Variant', 'வ?•ைப்படி')}</th>
                                              <th className="px-4 py-2.5">{l('Size / Weight', '?…ளவு / எடை')}</th>
                                              <th className="px-4 py-2.5 text-center">{l('Qty', '?…ளவு')}</th>
                                          ...
                                          <tfoot className="bg-[#fbf8f2] border-t border-[#e4d6c5]/30">
                                            <tr>
                                              <td colSpan={5} className="px-4 py-2.5 text-right font-black text-[#374151] text-[11px] uppercase tracking-wider">{l('Grand Total', 'மொத்த தொ?•ை')}</td>
                                    ...
                                    <div className="bg-white rounded-xl border border-blue-100 p-4">
                                      <div className="flex items-center justify-between mb-2">
                                        <span className="text-[11px] font-black text-[#374151] uppercase tracking-wider">{l('WhatsApp Message', 'வாட்ஸ் ?…ப் செய்தி')}</span>
                                        <button
                                          type="button"
                                          ...
                                          {l('Copy Message', 'ந?•ல் எடு')}
                                        </button>
                                      </div>
                                      <pre className="text-[12px] text-[#111111] bg-[#fbf8f2] rounded-xl p-3 whitespace-pre-wrap font-sans leading-relaxed select-all">{waMsg}</pre>
                                    </div>
                                  </div>
                                </td>
                              </tr>
                ...
                <div className="px-5 py-12 text-center">
                  ...
                  <p className="text-[14px] font-bold text-[#374151]">{l('No WhatsApp requests in selected period', 'தேர்ந்த ?•ாலத்தில் WA ?•ோரி?•்?•ை இல்லை')}</p>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-4 shadow-sm">
                <h3 className="text-[13px] font-black text-[#111111] mb-3">{l('Top Requested Products', '?…தி?• தேவை')}</h3>
              ...
              <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-4 shadow-sm">
                <h3 className="text-[13px] font-black text-[#111111] mb-3">{l('Top Categories', 'வ?•ை?•ள்')}</h3>
              ...
              <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-4 shadow-sm">
                <h3 className="text-[13px] font-black text-[#111111] mb-3">{l('Status Distribution', 'நிலை விள?•்?•ம்')}</h3>
                <div className="h-36">
        ...
        {tab === 'pos_analytics' && (
          <div className="space-y-6">
            ...
            {posAnalyticsTab === 'revenue' && (
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
                  {[
                    { label: 'TOTAL REVENUE',    helper: 'POS + manual combined', value: formatCurrency(analytics.totalCompletedRevenue), icon: <CurrencyIcon size={16} />, color: 'text-emerald-500', bg: 'bg-emerald-50' },
                    ...
                    { label: 'OFFLINE BILLS',    helper: 'Walk-in POS sales',     value: formatCurrency(analytics.posRevenue),            icon: <CurrencyIcon size={16} />, color: 'text-cyan-500',    bg: 'bg-cyan-50' },
                    { label: 'ONLINE BILLS',     helper: 'Online POS sales',      value: formatCurrency(analytics.onlinePosRevenue),      icon: <CurrencyIcon size={16} />, color: 'text-indigo-500',  bg: 'bg-indigo-50' },
                  ].map((card, index) => (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-5 gap-4">
                  {[
                    ...
                    { label: 'AVERAGE REVENUE PER BILL', helper: 'Average per Bill', value: formatCurrency(analytics.averageRevenuePerBill), icon: <CurrencyIcon size={16} />, color: 'text-emerald-500', bg: 'bg-emerald-50' },
                    ...
                    <div className="h-48">
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={analytics.monthlyTrend}>
                          ...
                          <YAxis hide />
                          <Tooltip cursor={{ fill: '#fbf8f2' }} formatter={(value) => formatCurrency(toNumber(value as number | string, 0))} />
                          <Bar dataKey="revenue" fill="#5a3928" radius={[4, 4, 0, 0]} barSize={12} />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  <div className="xl:col-span-2 bg-white rounded-card border border-borderLight p-6 shadow-soft">
                    <div className="flex items-center justify-between gap-3 mb-6">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="rounded-full bg-[#fbf8f2] px-3 py-1 text-[11px] font-bold text-[#5a3928]">
                  ...
                  <div className="bg-white rounded-card border border-borderLight p-6 shadow-soft">
                    ...
                    <div className="space-y-3">
                      {analytics.topProducts.slice(0, 5).map((p, i) => (
                        <div key={`${p.name}-${i}`} className="flex items-center justify-between rounded-xl bg-[#fbf8f2] p-3">
            ...
            {posAnalyticsTab === 'today' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                  {[
                    { label: "TODAY'S REVENUE", value: formatCurrency(analytics.todaySales), icon: <CurrencyIcon size={20} className="text-emerald-700" />, bg: 'bg-emerald-50', border: 'border-emerald-100' },
                ...
                <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                    {analytics.todayTopProducts.map((p, i) => (
                      <div key={i} className="flex items-center justify-between bg-[#fbf8f2] p-3 rounded-xl border border-gray-100">
                        <div className="min-w-0 flex-1">
                          <p className="text-[13px] font-bold text-[#111111] truncate">{p.name}</p>
                          <p className="text-[11px] text-[#374151]">{Math.round(p.qty)} sold</p>
                        </div>
                        <p className="text-[13px] font-black text-[#8a5b3d] ml-2">{formatCurrency(p.revenue)}</p>
                      </div>
                    ))}
                ...
                <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className="text-[11px] font-bold text-[#8a5b3d]">{analytics.todayCompletedOrdersCount} orders</span>
                  </div>
                  <div className="mb-3">
                    <input
                      type="text"
                      ...
                      className="w-full px-4 py-2.5 bg-[#fbf8f2] border border-[#e4d6c5]/60 rounded-xl text-[13px] font-bold text-[#111111] placeholder:text-[#8A9384] focus:outline-none focus:border-[#5a3928] transition-colors"
                    />
                  </div>
                  {(() => {
                          ...
                          {filteredBills.map(o => {
                            ...
                            return (
                              <tr key={o.id} className="hover:bg-[#fbf8f2]/50">
                                ...
                                <td className="px-3 py-2.5"><span className={`px-2 py-0.5 rounded-full text-[10px] font-black ${btClass}`}>{btLabel}</span></td>
                                <td className="px-3 py-2.5">
                                  <button onClick={() => void openOrderInvoice(o, 'view')} className="inline-flex items-center gap-1 rounded-lg border border-[#e4d6c5]/60 px-2 py-1.5 text-[11px] font-black text-[#111111] hover:bg-[#fbf8f2]" title="View Invoice">
            ...
            {posAnalyticsTab === 'products' && (
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    { label: 'Total Product Revenue', value: formatCurrency(analytics.totalCompletedRevenue), icon: <CurrencyIcon size={18} />, from: 'from-emerald-500 to-teal-600' },
                    ...
                    { label: 'Average Product Revenue', value: `${formatCurrency(analytics.averageProductRevenue)} / Product`, icon: <CurrencyIcon size={18} />, from: 'from-violet-500 to-purple-600' },
                ...
                <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                    ...
                    <span className="text-[11px] font-bold text-[#8a5b3d] bg-emerald-50 px-3 py-1 rounded-full border border-emerald-200 self-start sm:self-auto">{analytics.topProducts.length} products</span>
                  </div>
                  <div className="mb-4">
                    <input
                      type="text"
                    ...
                    return filteredProds.length > 0 ? (
                      ...
                      <div className="space-y-3 md:hidden">
                        {filteredProds.slice(0, 50).map((p, i) => (
                          <div key={`${p.name}-${p.variant || i}`} className="rounded-2xl border border-[#e4d6c5]/30 bg-[#f5f0e8] p-4">
                      ...
                      <div className="hidden md:block overflow-x-auto rounded-xl border border-[#e4d6c5]/30">
                        <table className="w-full min-w-[580px] text-left text-[12px]">
                          ...
                          <tbody className="divide-y divide-[#e4d6c5]/20">
                            {filteredProds.slice(0, 50).map((p, i) => (
                              <tr key={`${p.name}-${p.variant || i}`} className="hover:bg-[#fbf8f2]/50">
            ...
            {posAnalyticsTab === 'categories' && (
              <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                <h3 className="text-base font-black text-[#111111] mb-4">{l('Category Analytics', 'வ?•ை ப?•ுப்பாய்வு')}</h3>
                {analytics.topCategories.length > 0 ? (
                  <>
                  <div className="space-y-3 md:hidden">
                    {analytics.topCategories.map((c, i) => (
                      <div key={c.name} className="rounded-2xl border border-[#e4d6c5]/30 bg-[#f5f0e8] p-4">
                        <div className="flex items-start justify-between gap-3">
                          <div>
                            <p className="text-[13px] font-black text-[#9BAB9A]">#{i + 1}</p>
                  ...
                  <div className="hidden md:block overflow-x-auto rounded-xl border border-[#e4d6c5]/30">
                    <table className="w-full text-left text-[13px]">
                      <thead className="bg-[#fbf8f2] text-[10px] uppercase tracking-wider text-[#374151]">
                        <tr>
                          <th className="px-4 py-2.5 font-black">#</th>
                          <th className="px-4 py-2.5 font-black">{l('Category', 'வ?•ை')}</th>
                          <th className="px-4 py-2.5 font-black">{l('Revenue', 'வருவாய்')}</th>
                          <th className="px-4 py-2.5 font-black">{l('Qty Sold', 'விற்ற ?…ளவு')}</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-[#e4d6c5]/20">
                        {analytics.topCategories.map((c, i) => (
                          <tr key={c.name} className="hover:bg-[#fbf8f2]/50">
                            <td className="px-4 py-2 text-[11px] text-[#9BAB9A] font-bold">{i + 1}</td>
                  ...
                  <p className="text-center text-[13px] text-[#374151] py-6">{l('No data in selected period', 'தேர்ந்த ?•ாலத்தில் தரவு இல்லை')}</p>
                )}
              </div>
            )}
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                  {[
                    ...
                    { label: 'Total Discounts Given', value: formatCurrency(analytics.totalCouponDiscounts), icon: <CurrencyIcon size={18} />, from: 'from-blue-500 to-indigo-600' },
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
                  <div className="xl:col-span-2 bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                      ...
                      <div className="h-64">
                        <ResponsiveContainer width="100%" height="100%">
                          <BarChart data={analytics.couponDailyTrend}>
                            ...
                            <XAxis dataKey="day" tick={{ fill: '#6B7280', fontSize: 11, fontWeight: 700 }} axisLine={false} tickLine={false} />
                            <YAxis hide />
                            <Tooltip cursor={{ fill: '#fbf8f2' }} formatter={(value, name) => [value, name === 'orders' ? 'Orders' : 'Discounts']} />
                            <Bar dataKey="orders" fill="url(#couponGrad)" radius={[4, 4, 0, 0]} barSize={24} />
                            <defs>
                              <linearGradient id="couponGrad" x1="0" y1="0" x2="0" y2="1">
                                <stop offset="0%" stopColor="#8a5b3d" />
                  ...
                  <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                    ...
                    <div className="space-y-3">
                      {analytics.topCoupons.slice(0, 8).map((coupon, i) => (
                        <div key={coupon.code} className="flex items-center justify-between gap-2 p-3 bg-[#fbf8f2] rounded-xl">
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2">
                ...
                {analytics.topCoupons.length > 0 && (
                  <div className="bg-white rounded-2xl border border-[#e4d6c5]/30 p-5 shadow-sm">
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <span className="text-[11px] font-bold text-[#8a5b3d]">{analytics.topCoupons.length} coupons</span>
                    </div>
                    <div className="space-y-3 md:hidden">
                      {analytics.topCoupons.map((coupon, i) => (
                        <div key={coupon.code} className="rounded-2xl border border-[#e4d6c5]/30 bg-[#f5f0e8] p-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <p className="text-[13px] font-black text-[#9BAB9A]">#{i + 1}</p>
                    ...
                    <div className="hidden md:block overflow-x-auto rounded-xl border border-[#e4d6c5]/30">
                      <table className="w-full min-w-[400px] text-left text-[12px]">
                        ...
                        <tbody className="divide-y divide-[#e4d6c5]/20">
                          {analytics.topCoupons.map((coupon, i) => (
                            <tr key={coupon.code} className="hover:bg-[#fbf8f2]/50">
                              <td className="px-4 py-2 text-[11px] text-[#9BAB9A] font-bold">{i + 1}</td>
        ...
        {/* ── ORDER MANAGEMENT ── */}
        {tab === 'history' && (
          <div className="space-y-4 sm:space-y-6 rounded-[20px] sm:rounded-[28px] border border-[#e4d6c5]/60 bg-[#f5f0e8] p-3 sm:p-6 lg:p-7 shadow-sm">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <p className="text-[11px] font-black uppercase tracking-[0.24em] text-[#8a5b3d]">{l('Billing history', 'பில் வரலாறு')}</p>
                <h2 className="mt-1 text-xl font-black text-[#111111]">{l('Order Management', 'ஆர்டர் மேலாண்மை')} <span className="text-[11px] font-semibold text-[#374151]">({l('POS Bills only', 'POS பில்?•ள் மட்டுமே')})</span></h2>
              </div>
            ...
            <div className="rounded-2xl border border-[#e4d6c5]/60 bg-white p-3 sm:p-6 shadow-sm">
              {/* Bill type filter */}
              <div className="flex flex-wrap gap-2 mb-4">
                {([
                  { v: 'all',     l: l('All Bills', '?…?©ைத்து') },
                  { v: 'offline', l: l('Offline', 'ஆஃப்லை?©்') },
                  { v: 'online',  l: l('Online', 'ஆ?©்லை?©்') },
                  { v: 'manual',  l: l('Manual', '?•ைமுறை') },
                ] as const).map(({ v, l }) => (
                  <button key={v} type="button" onClick={() => setBillTypeFilter(v)}
                    className={`min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black transition-colors ${billTypeFilter === v ? 'bg-[#111111] text-white shadow-sm' : 'bg-[#fbf8f2] text-[#374151] hover:bg-[#e4d6c5]/40'}`}>
                    {l}
                  </button>
                ))}
              </div>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                  {(['today', 'week', 'month', 'custom'] as const).map(preset => (
                    <button key={preset} type="button" onClick={() => applyDatePreset(preset)}
                      className={`min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black transition-colors ${datePreset === preset ? 'bg-[#5a3928] text-white shadow-sm' : 'bg-[#fbf8f2] text-[#374151] hover:bg-[#e4d6c5]/40'}`}>
                      {preset === 'today' ? l('Today','இ?©்று') : preset === 'week' ? l('This Week','இந்த வாரம்') : preset === 'month' ? l('This Month','இந்த மாதம்') : l('Custom Range','தேர்வு')}
                    </button>
                  ))}
                  {(search.dateFrom || search.dateTo || datePreset) && (
                    <button type="button" onClick={() => { setDatePreset(''); setSearch(s => ({ ...s, dateFrom: '', dateTo: '' })) }}
                      className="min-h-[44px] px-3 py-1.5 rounded-xl text-[12px] font-black text-[#5a3928] hover:bg-[#5a3928]/5">{l('Clear Dates', 'தேதி ?…ழி')}</button>
                  )}
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                  <input className="min-h-[48px] rounded-xl bg-[#fbf8f2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#111111] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#5a3928]/15" placeholder={l('Invoice / Bill No', 'பில் எண்')}
                    value={search.invoiceNo} onChange={e => setSearch(s => ({ ...s, invoiceNo: e.target.value }))} />
                  <input className="min-h-[48px] rounded-xl bg-[#fbf8f2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#111111] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#5a3928]/15" placeholder={l('Customer Name', 'வாடி?•்?•ையாளர் பெயர்')}
                    value={search.customerName} onChange={e => setSearch(s => ({ ...s, customerName: e.target.value }))} />
                  <input className="min-h-[48px] rounded-xl bg-[#fbf8f2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#111111] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#5a3928]/15" placeholder={l('Mobile Number', 'மொபைல் எண்')}
                    value={search.phone} onChange={e => setSearch(s => ({ ...s, phone: e.target.value }))} />
                  <input className="min-h-[48px] rounded-xl bg-[#fbf8f2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#111111] placeholder:text-[#8A9384] focus:outline-none focus:ring-2 focus:ring-[#5a3928]/15" placeholder={l('Reference Number', '?•ுறிப்பு எண்')}
                    value={search.referenceNumber} onChange={e => setSearch(s => ({ ...s, referenceNumber: e.target.value }))} />
                  {datePreset === 'custom' ? (
                    <>
                      <input type="date" className="min-h-[48px] rounded-xl bg-[#fbf8f2] px-3 py-2.5 text-[16px] md:text-[13px] font-semibold text-[#111111] focus:outline-none focus:ring-2 focus:ring-[#5a3928]/15"
                    ...
                    <button type="submit" disabled={searchLoading}
                      className="sm:col-span-2 min-h-[48px] flex items-center justify-center gap-2 rounded-xl bg-[#5a3928] py-2.5 text-[13px] font-bold text-white shadow-sm transition-colors hover:bg-[#065F46] disabled:opacity-60">
                      <Search size={14} /> {searchLoading ? l('Searching...','தேடு?•ிறது...') : l('Search Bills','தேடு')}
                    </button>
                  )}
                  {datePreset === 'custom' && (
                    <button type="submit" disabled={searchLoading}
                      className="sm:col-span-2 lg:col-span-4 min-h-[48px] flex items-center justify-center gap-2 rounded-xl bg-[#5a3928] py-2.5 text-[13px] font-bold text-white shadow-sm transition-colors hover:bg-[#065F46] disabled:opacity-60">
                      <Search size={14} /> {searchLoading ? l('Searching...','தேடு?•ிறது...') : l('Search Bills','தேடு')}
                    </button>
                  )}
                </div>
              ...
              <div className="flex items-center justify-between mb-2">
                <p className="text-[11px] font-semibold text-[#374151]">{filteredSearchResults.length} {l('result(s)', 'முடிவு?•ள்')}</p>
                {filteredSearchResults.length > 0 && (
                  <button onClick={() => exportXlsx(filteredSearchResults)}
                    className="flex items-center gap-1 text-[11px] font-bold text-[#5a3928] hover:underline">
              ...
              <div className="space-y-3 md:hidden">
                {filteredSearchResults.slice(0, 50).map(o => {
                  ...
                  return (
                    <div key={o.id} className="rounded-2xl border border-[#e4d6c5]/60 bg-[#f5f0e8] p-3 sm:p-4 space-y-3">
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                      ...
                      <div className="flex flex-col sm:flex-row gap-2 pt-1">
                        <div className="flex gap-2 w-full sm:flex-1">
                          <button onClick={() => void openOrderInvoice(o, 'view')} className="inline-flex h-10 sm:min-h-[44px] flex-1 items-center justify-center gap-1.5 rounded-xl border border-[#e4d6c5]/60 px-2 sm:px-3 text-[12px] font-black text-[#111111] transition-colors hover:bg-white" title="View Invoice">
                            <Eye size={14} /> View
                          </button>
                        ...
                        <button onClick={() => void deleteOrder(o.id, o.invoice_no)} className="h-10 w-10 sm:h-11 sm:w-11 shrink-0 rounded-xl border border-[#e4d6c5]/60 text-[#5a3928] transition-colors hover:bg-[#5a3928]/5" title="Delete Order">
                ...
                {filteredSearchResults.length === 0 && (
                  <div className="rounded-2xl border border-[#e4d6c5]/60 bg-[#f5f0e8] px-4 py-8 text-center text-[#374151]">{l('No matching bills', 'பில்?•ள் இல்லை')}</div>
                )}
              </div>
              <div className="hidden md:block overflow-x-auto rounded-xl border border-[#e4d6c5]/60 bg-[#f5f0e8]">
                <table className="w-full text-left text-[13px]">
                  <thead className="bg-[#fbf8f2] text-[10px] uppercase tracking-wider text-[#374151]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#e4d6c5]/30 bg-white">
                    {filteredSearchResults.slice(0, 50).map(o => {
                      const billTypeLabel = normalizeOrderType(o.order_type) === 'manual_sale' ? 'MANUAL' : normalizeOrderMode(o.order_mode) === 'online' ? 'ONLINE' : 'OFFLINE'
                      const billTypeClass = normalizeOrderType(o.order_type) === 'manual_sale' ? 'bg-purple-50 text-purple-700' : normalizeOrderMode(o.order_mode) === 'online' ? 'bg-blue-50 text-blue-700' : 'bg-orange-50 text-orange-700'
                      return (
                        <React.Fragment key={o.id}>
                        <tr key={o.id} className="hover:bg-[#fbf8f2] text-center">
                          ...
                          <td className="px-2 py-3">
                            <div className="flex items-center justify-center gap-1.5">
                              ...
                              <button onClick={() => void deleteOrder(o.id, o.invoice_no)} className="rounded-lg p-1 text-[#5a3928] transition-colors hover:bg-[#5a3928]/5" title="Delete Order">
                                <Trash2 size={13} />
                              </button>
                            </div>
                          </td>
                          <td className="px-2 py-3">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => void openOrderInvoice(o, 'view')} className="rounded-lg p-1 text-[#111111] transition-colors hover:bg-[#fbf8f2]" title="View Invoice">
                                <Eye size={13} />
                              </button>
                        ...
                        {historyExpandedId === o.id && (
                          <tr className="bg-[#fbf8f2] border-b border-[#e4d6c5]/40">
                    ...
                    {filteredSearchResults.length === 0 && (
                      <tr><td colSpan={12} className="px-4 py-8 text-center text-[#374151]">{l('No matching bills', 'பில்?•ள் இல்லை')}</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-5 gap-6">
            ...
            <div className="xl:col-span-2">
              <form onSubmit={handleSaveProd} className="bg-white rounded-2xl border border-borderLight p-6 shadow-sm space-y-5">
                <h3 className="text-[18px] font-black text-[#111111]">{editingProd ? l('Edit Product', 'திருத்து') : l('Add Product', 'சேர்?•்?•வும்')}</h3>
                ...
                {/* Product Type */}
                <div>
                  <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-2">{l('Product Type', 'பொருள் வ?•ை')} *</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="col-span-2">
                    ...
                    <input className="w-full px-4 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark rounded-xl text-[13px] font-bold outline-none transition-colors"
                      placeholder="எ.?•ா. மஞ்சள் பொடி" value={prodForm.nameTa} onChange={e => setProdForm(f => ({...f, nameTa: e.target.value}))} />
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">{l('Purchase Price (₹)', 'வாங்?•ிய விலை')} *</label>
                    <input required type="number" min="0" step="0.01"
                      className="w-full px-4 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark rounded-xl text-[13px] font-bold outline-none transition-colors"
                  ...
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">{l('Offer Price (₹)', 'சலு?•ை விலை')}</label>
                    <input type="number" min="0" step="0.01"
                      className="w-full px-4 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark rounded-xl text-[13px] font-bold outline-none transition-colors"
                  ...
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">Pricing mode</label>
                    <select value={prodForm.pricingType} onChange={e => setProdForm(f => ({ ...f, pricingType: e.target.value as 'fixed' | 'manual' }))} className="w-full px-3 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] rounded-xl text-[13px] font-bold">
                      <option value="fixed">Fixed price</option><option value="manual">Manual price at sale</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">Catalog type</label>
                    <select value={prodForm.productType} onChange={e => setProdForm(f => ({ ...f, productType: e.target.value as 'standard' | 'lens' | 'other' }))} className="w-full px-3 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] rounded-xl text-[13px] font-bold">
                      <option value="standard">Standard product</option><option value="lens">Lens</option><option value="other">Other</option>
                    </select>
                  </div>
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">Brand type</label>
                    <select value={prodForm.brandType} onChange={e => setProdForm(f => ({ ...f, brandType: e.target.value as 'none' | 'non_branded' | 'branded' }))} className="w-full px-3 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] rounded-xl text-[13px] font-bold">
                      <option value="none">No brand</option><option value="non_branded">Non-branded</option><option value="branded">Branded</option>
                    </select>
                  </div>
                  <div>
                  ...
                  <div>
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">{l('Barcode', 'பார்?•ோடு')}</label>
                    <input className="w-full px-4 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark rounded-xl text-[13px] font-bold outline-none transition-colors"
                  ...
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">{l('Category', 'வ?•ை')} *</label>
                    <select required className="w-full min-w-0 h-11 px-4 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark rounded-xl text-[13px] font-bold outline-none transition-colors touch-manipulation"
                      ...
                      <option value="">{l('Select category...', 'வ?•ை தேர்வு செய்யுங்?•ள்...')}</option>
                    ...
                    {categoryManagerOpen && (
                      <div className="mt-2 rounded-xl border border-[#e4d6c5]/60 bg-white p-2 space-y-1">
                        {cats.length === 0 ? (
                          ...
                          <div key={c.id} className="relative flex items-center justify-between gap-2 rounded-lg px-2 py-1.5 hover:bg-red-50">
                            ...
                            {openCategoryMenuId === c.id && (
                              <div className="absolute right-2 top-9 z-20 min-w-28 rounded-xl border border-[#e4d6c5]/60 bg-white p-1 shadow-lg">
                                <button type="button" onClick={() => { setEditingCategoryId(c.id); setNewCat({ name_en: c.name_en, name_ta: c.name_ta || '' }); setOpenCategoryMenuId(null) }} className="block w-full rounded-lg px-3 py-2 text-left text-[11px] font-bold text-[#111111] hover:bg-[#fbf8f2]">Edit / Rename</button>
                ...
                {(prodForm.unitType === 'weight' || prodForm.unitType === 'volume') && (
                  <div>
                    ...
                    <p className="text-[11px] text-[#6B7280] mt-1">{l('These become the selectable size buttons on the product card.', 'இவை பொருள் ?…ட்டையில் ?…ளவு பொத்தா?©்?•ளா?• ?•ாட்டப்படும்.')}</p>
                  </div>
                )}

                <div>
                  <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">{l('Description', 'விள?•்?•ம்')}</label>
                ...
                <div>
                  <label className="block text-[11px] font-black uppercase text-[#6B7280] tracking-wider mb-1">{l('Benefits / Health Tags', 'ந?©்மை?•ள்')}</label>
                  <input className="w-full px-4 py-2.5 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark rounded-xl text-[13px] font-bold outline-none transition-colors"
                ...
                <div className="space-y-3">
                  ...
                  {imageUploading && <p className="text-[12px] text-maroon-dark font-bold">{l('Uploading image...', 'படம் பதிவேற்று?•ிறது...')}</p>}
                  {prodForm.image && (
                ...
                <div className="flex items-center gap-3 pt-2">
                  ...
                  />
                  <label htmlFor="isActive" className="text-[14px] font-bold text-[#111111]">{l('Active (visible in store)', '?•டையில் ?•ாட்டு')}</label>
                </div>
                <div className="flex items-center gap-3">
                  ...
                  <label htmlFor="hasVariants" className="text-[14px] font-bold text-[#111111]">
                    {l('Has Variants (brands/sizes)', 'வ?•ை?•ள் உள்ள?©')}
                  </label>
                </div>
                ...
                <div className="flex gap-3 pt-3 border-t border-borderLight">
                  <button type="submit" disabled={loading}
                    ...
                    {loading ? l('Saving...','சேமி?•்?•ிறது...') : editingProd ? l('Update Product','புதுப்பி') : l('Add Product','சேர்?•்?•வும்')}
                  </button>
            ...
            <div className="xl:col-span-3">
              <div className="bg-white rounded-2xl border border-borderLight shadow-sm overflow-hidden flex flex-col h-full">
                <div className="px-6 py-5 border-b border-borderLight flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white">
                  <div>
                    <h3 className="text-[18px] font-black text-[#111111]">{l('Products', 'பொருட்?•ள்')} <span className="text-[#6B7280] font-medium text-[16px]">({products.length})</span></h3>
                  </div>
                  <div className="flex items-center gap-3">
                    <div className="relative">
                      ...
                      <input
                        type="text"
                        placeholder={l('Search items...', 'பொருட்?•ளை தேட...')}
                  ...
                  <table className="w-full min-w-[640px] text-left border-collapse">
                    <thead className="bg-[#FAFAFA] text-[11px] uppercase tracking-wider text-[#6B7280] border-b border-borderLight">
                      <tr>
                        ...
                        <th className="px-4 py-4 font-black">{l('Type', 'வ?•ை')}</th>
                        <th className="px-4 py-4 font-black">{l('Stock', 'இருப்பு')}</th>
                        <th className="px-4 py-4 font-black">{l('Price', 'விலை')}</th>
                        <th className="px-6 py-4 font-black text-right">{l('Actions', 'நடவடி?•்?•ை')}</th>
                      </tr>
                    </thead>
              ...
              <div className="xl:col-span-5 bg-white rounded-2xl border border-borderLight p-6 shadow-sm">
                <div className="flex items-center justify-between mb-6">
                  <h3 className="text-[18px] font-black text-[#111111]">
                    {l('Variants', 'வ?•ை?•ள்')} - <span className="text-[#6B7280]">{editingProd.name}</span>
                    {!editingProd.hasVariants && (
                      <span className="ml-3 text-[12px] font-bold text-amber-700 bg-amber-50 px-3 py-1 rounded-full">
                        {l('Enable "Has Variants" above to manage variants', '"வ?•ை?•ள் உள்ள?©" இய?•்?•வும்')}
                      </span>
                    )}
                  </h3>
                  {variantNotice && (
                ...
                <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
                  ...
                  <form onSubmit={handleSaveVariant} className="space-y-4 bg-[#FAFAFA] rounded-2xl p-5 border border-[#F3F4F6]">
                    <h4 className="text-[13px] font-black uppercase tracking-wider text-[#111111]">
                      {editingVariantId ? l('Edit Variant', 'வ?•ை திருத்து') : l('Add Variant', 'வ?•ை சேர்')}
                    </h4>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="col-span-2">
                        <label className="block text-[11px] font-black uppercase tracking-wider text-[#6B7280] mb-1">{l('Variant Name *', 'வ?•ை பெயர் *')}</label>
                        <input required
                      ...
                      <div>
                        <label className="block text-[11px] font-black uppercase tracking-wider text-[#6B7280] mb-1">{l('Size Label', '?…ளவு பட்டை')}</label>
                        <input
                          className="w-full px-4 py-2.5 bg-white rounded-xl border border-[#D1D5DB] text-[13px] font-bold outline-none focus:border-maroon-dark transition-colors shadow-sm"
                      ...
                      <div>
                        <label className="block text-[11px] font-black uppercase tracking-wider text-[#6B7280] mb-1">{l('Purchase Price (₹)', 'வாங்?•ிய விலை')}</label>
                        <input type="number" min="0" step="0.01"
                          className="w-full px-4 py-2.5 bg-white rounded-xl border border-[#D1D5DB] text-[13px] font-bold outline-none focus:border-maroon-dark transition-colors shadow-sm"
                          placeholder="30"
                      ...
                      <div>
                        <label className="block text-[11px] font-black uppercase tracking-wider text-[#6B7280] mb-1">{l('Selling Price (₹) *', 'விற்ப?©ை விலை *')}</label>
                        <input required type="number" min="0" step="0.01"
                          className="w-full px-4 py-2.5 bg-white rounded-xl border border-[#D1D5DB] text-[13px] font-bold outline-none focus:border-maroon-dark transition-colors shadow-sm"
                          placeholder="40"
                      ...
                      <div>
                        <label className="block text-[11px] font-black uppercase tracking-wider text-[#6B7280] mb-1">{l('Barcode', 'பார்?•ோடு')}</label>
                        <input className="w-full px-4 py-2.5 bg-white rounded-xl border border-[#D1D5DB] text-[13px] font-bold outline-none focus:border-maroon-dark transition-colors shadow-sm"
                          placeholder="890..."
                      ...
                      <div>
                        <label className="block text-[11px] font-black uppercase tracking-wider text-[#6B7280] mb-1">{l('Unit', '?…ல?•ு')}</label>
                        <select
                    ...
                    <div className="flex items-center gap-3 pt-2">
                      <input type="checkbox" id="varIsDefault" checked={variantForm.isDefault}
                        ...
                        className="w-4 h-4 text-maroon-dark rounded focus:ring-maroon-dark accent-maroon-dark"
                      />
                      <label htmlFor="varIsDefault" className="text-[13px] font-bold text-[#111111]">{l('Default variant (shown first)', 'முதல் வ?•ை (முதலில் ?•ாட்டு)')}</label>
                    </div>
                    ...
                    <div className="flex gap-3 pt-2">
                      <button type="submit" disabled={variantLoading}
                        ...
                        {variantLoading ? l('Saving...', 'சேமி?•்?•ிறது...') : editingVariantId ? l('Update Variant', 'புதுப்பி') : l('Add Variant', 'சேர்')}
                      </button>
                      {editingVariantId && (
                        <button type="button"
                  ...
                  <div className="bg-[#FAFAFA] rounded-2xl p-5 border border-[#F3F4F6]">
                    <h4 className="text-[13px] font-black uppercase tracking-wider text-[#111111] mb-4">
                      {l('Current Variants', 'தற்போதைய வ?•ை?•ள்')} <span className="text-[#6B7280]">({getVariants(String(editingProd.id)).length})</span>
                    </h4>
                    {getVariants(String(editingProd.id)).length === 0 ? (
                      <p className="text-[13px] text-[#6B7280] text-center py-8 bg-white border border-[#F3F4F6] rounded-xl">
                        {l('No variants yet - add one using the form.', 'வ?•ை?•ள் இல்லை - படிவத்தில் சேர்?•்?•வும்.')}
                      </p>
                    ) : (
        ...
        {/* ── CATEGORIES TAB ── */}
        {tab === 'catalog_pricing' && <CatalogPricingPanel />}

        {tab === 'categories' && (
          <div className="w-full max-w-3xl space-y-6">
            <div className="bg-white rounded-2xl border border-borderLight p-4 sm:p-6 shadow-sm">
              <h3 className="text-[18px] font-black text-[#111111] mb-5">{l('Product Categories', 'பொருள் வ?•ை?•ள்')}</h3>
              {categoryNotice && (
              ...
              <form onSubmit={onAddCat} className="mb-6 grid grid-cols-1 gap-3 sm:grid-cols-[minmax(0,1fr)_minmax(120px,0.45fr)_auto]">
                <input className="min-w-0 w-full px-4 py-3 bg-[#FAFAFA] border border-[#F3F4F6] focus:border-maroon-dark outline-none rounded-xl text-[13px] font-bold transition-colors shadow-sm"
                  placeholder={l('Category name (English)', 'வ?•ை பெயர் (English)')} value={newCat.name_en}
        ...
        {tab === 'coupons' && (
            ...
            <div className="flex flex-col gap-3">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div className="space-y-1">
                  <h2 className="text-[22px] lg:text-[24px] leading-none font-black text-[#111111]">{l('Coupon Management', '?•ூப்ப?©் மேலாண்மை')}</h2>
                  <p className="max-w-2xl text-[12px] lg:text-[12px] font-medium text-[#6C665C]">
                    {l('Create and manage discount codes. Applies to product subtotal only.', 'பொருட்?•ளி?©் subtotal-?•்?•ு மட்டும் ?•ூப்ப?©் தள்ளுபடி பொருந்தும்.')}
                  </p>
                </div>
                <button
                  onClick={() => void loadCoupons()}
                  className="inline-flex items-center gap-2 rounded-full border border-[#e4d6c5] bg-[#f5f0e8] px-3 py-2 text-[11px] font-black text-[#5a3928] shadow-sm transition-colors hover:bg-[#F7F1E7]"
                >
                  <RefreshCw size={12} />
                  Refresh
                </button>
              ...
              <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
                <div className="rounded-xl border border-[#e4d6c5] bg-[#f5f0e8] px-3 py-3 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#5a3928]">Total Coupons</p>
                  <p className="mt-1 text-[20px] font-black text-[#111111]">{coupons.length}</p>
                </div>
                <div className="rounded-xl border border-[#e4d6c5] bg-[#f5f0e8] px-3 py-3 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#5a3928]">Active</p>
                  <p className="mt-1 text-[20px] font-black text-[#5a3928]">{coupons.filter(c => c.is_active).length}</p>
                </div>
                <div className="rounded-xl border border-[#e4d6c5] bg-[#f5f0e8] px-3 py-3 shadow-sm">
                  <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#5a3928]">Used</p>
                  <p className="mt-1 text-[20px] font-black text-[#111111]">{coupons.reduce((acc, c) => acc + (c.usage_count || 0), 0)}</p>
                </div>
              </div>

              <div className="rounded-xl border border-[#E7CFAA] bg-[#FFF6E7] px-3 py-2 text-[11px] font-bold text-[#5a3928] shadow-sm">
                {l('Coupon discount applies to product subtotal only - not delivery charge.', '?•ூப்ப?©் தள்ளுபடி பொருட்?•ளி?©் subtotal-?•்?•ு மட்டும் பொருந்தும்.')}
              </div>
            </div>

            <div className="grid grid-cols-1 gap-4 xl:grid-cols-[380px_minmax(0,1fr)]">
              <form onSubmit={saveCoupon} className="rounded-2xl border border-[#e4d6c5] bg-[#FFFCF6] p-4 shadow-sm space-y-4">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#5a3928]">{editingCouponId !== null ? 'Edit mode' : 'New coupon'}</p>
                    <h3 className="mt-1 text-[17px] font-black text-[#111111]">
                      {editingCouponId !== null ? l('Edit Coupon', '?•ூப்ப?©ை திருத்து') : l('Create Coupon', 'புதிய ?•ூப்ப?©்')}
                    </h3>
                  </div>
                  {editingCouponId !== null && (
                    <button
                      ...
                      className="rounded-full border border-[#E7CFAA] bg-[#FFF6E7] px-2.5 py-1 text-[11px] font-black text-[#5a3928] transition-colors hover:bg-[#FBEBD3]"
                    >
                      Cancel
                    </button>
                  )}
                ...
                <div className="space-y-1.5">
                  <label className="block text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280]">{l('Coupon Code', '?•ூப்ப?©் ?•ுறியீடு')} *</label>
                  <div className="flex gap-2">
                    <input
                      className="flex-1 rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-black uppercase tracking-[0.12em] text-[#111111] outline-none transition-colors focus:border-[#5a3928]"
                    ...
                    {editingCouponId === null && (
                      <button
                        ...
                        className="shrink-0 rounded-xl border border-[#5a3928] bg-[#5a3928] px-3 py-2.5 text-[11px] font-black text-white transition-colors hover:bg-[#741D2A]"
                      >
                        Generate
                      </button>
                    )}
                  ...
                  {editingCouponId !== null && (
                    <p className="text-[10px] font-medium text-[#6B7280]">{l('Code cannot be changed when editing', 'திருத்தும்போது ?•ுறியீட்டை மாற்ற முடியாது')}</p>
                  )}
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    ...
                    <select value={couponForm.discount_type} onChange={e => setCouponForm(f => ({ ...f, discount_type: e.target.value as 'percentage' | 'fixed' }))} className="mb-2 w-full rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-bold text-[#111111] outline-none">
                      <option value="percentage">Percentage</option><option value="fixed">Fixed amount</option>
                    </select>
                    <input
                      type="number"
                      min="0"
                      className="w-full rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-bold text-[#111111] outline-none transition-colors focus:border-[#5a3928]"
                      placeholder={couponForm.discount_type === 'percentage' ? '10%' : '500'}
                      value={couponForm.discount_value}
                      max={couponForm.discount_type === 'percentage' ? 100 : undefined}
                      onChange={e => setCouponForm(f => ({ ...f, discount_value: e.target.value, percentage: couponForm.discount_type === 'percentage' ? Number(e.target.value) : 0 }))}
                    />
                  </div>
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280]">{l('Min Order (₹)', '?•ுறைந்த ஆர்டர் (₹)')}</label>
                    <input
                      type="number"
                      min="0"
                      className="w-full rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-bold text-[#111111] outline-none transition-colors focus:border-[#5a3928]"
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280]">Max discount</label>
                    <input type="number" min="0" value={couponForm.maximum_discount} onChange={e => setCouponForm(f => ({ ...f, maximum_discount: e.target.value }))} placeholder="Optional cap" className="w-full rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-bold text-[#111111] outline-none" />
                  </div>
                </div>

                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280]">{l('Expiry Date', '?•ாலாவதி தேதி')}</label>
                    <input
                      type="date"
                      className="w-full rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-bold text-[#111111] outline-none transition-colors focus:border-[#5a3928]"
                  ...
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280]">{l('Usage Limit', 'பய?©்பாட்டு வரம்பு')}</label>
                    <input
                      type="number"
                      min="1"
                      className="w-full rounded-xl border border-[#A7F3D0] bg-white px-3 py-2.5 text-[12px] font-bold text-[#111111] outline-none transition-colors focus:border-[#5a3928]"
                ...
                <button
                  type="submit"
                  className="w-full rounded-xl bg-[#5a3928] py-3 text-[13px] font-black text-white shadow-sm transition-colors hover:bg-[#741D2A]"
                >
                  {editingCouponId !== null ? l('Update Coupon', '?•ூப்ப?©ை புதுப்பி') : l('Create Coupon', '?•ூப்ப?©ை உருவா?•்?•ு')}
                </button>
              </form>

              <div className="rounded-2xl border border-[#e4d6c5] bg-[#FFFCF6] p-4 shadow-sm">
                <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
                  <div>
                    <p className="text-[10px] font-black uppercase tracking-[0.15em] text-[#6B7280]">{l('Coupon List', '?•ூப்ப?©் பட்டியல்')}</p>
                    <h3 className="mt-1 text-[17px] font-black text-[#111111]">
                      {l('All Coupons', '?…?©ைத்து ?•ூப்ப?©்?•ள்')} <span className="text-[#6B7280]">({coupons.length})</span>
                    </h3>
                  </div>
                  <span className="rounded-full border border-[#E7CFAA] bg-[#FFF6E7] px-2.5 py-0.5 text-[10px] font-black uppercase tracking-[0.15em] text-[#5a3928]">
                    {l('Admin only', '?…ட்மி?©் மட்டும்')}
                  </span>
                </div>
                ...
                <div className="space-y-2 max-h-[30rem] overflow-y-auto pr-1">
                    ...
                    return (
                      <div
                        ...
                        className={`rounded-xl border p-3 shadow-sm transition-all ${
                          isEditing
                            ? 'border-[#5a3928] bg-[#FFF8F3] ring-1 ring-[#5a3928]/15'
                        ...
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div className="min-w-0 space-y-1.5">
                            <div className="flex flex-wrap items-center gap-1.5">
                              ...
                              <span className={`rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-[0.14em] ${coupon.is_active ? 'bg-[#FCE7EA] text-[#5a3928]' : 'bg-[#F8EDD9] text-[#9A6700]'}`}>
                            ...
                            <p className="text-[12px] font-semibold text-[#5a3928]">
                              {coupon.discount_type === 'fixed' ? `${formatCurrency(Number(coupon.discount_value || 0))} off` : `${coupon.discount_value || coupon.percentage}% off`}
                              {coupon.min_order_value > 0 && ` • min ${formatCurrency(Number(coupon.min_order_value))}`}
                            </p>
                          ...
                          <div className="flex shrink-0 items-center gap-1.5">
                            <button
                              ...
                              className={`rounded-full px-2.5 py-1.5 text-[10px] font-black uppercase tracking-[0.14em] transition-colors ${
                                coupon.is_active ? 'bg-[#FCE7EA] text-[#5a3928] hover:bg-[#F8D7DD]' : 'bg-[#F8EDD9] text-[#9A6700] hover:bg-[#F2E0B9]'
                              }`}
                            >
                            ...
                            <button
                              ...
                              className="rounded-full border border-[#A7F3D0] bg-white p-2 text-[#5a3928] transition-colors hover:border-[#D8BA8A] hover:text-[#741D2A]"
                            >
                              <Edit2 size={14} />
                            </button>
                            <button
                  ...
                  {coupons.length === 0 && (
                    <div className="rounded-[22px] border border-dashed border-[#E7CFAA] bg-[#FFF8F3] py-12 text-center text-[14px] font-bold text-[#5a3928]">
                      {l('No coupons yet. Create your first coupon!', 'இ?©்?©ும் ?•ூப்ப?©் இல்லை. முதல் ?•ூப்ப?©ை உருவா?•்?•ுங்?•ள்!')}
                    </div>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-6">
            <div className="flex flex-wrap items-center justify-between gap-3">
              <h2 className="text-[20px] font-black text-[#111111]">{l('User Management', 'பய?©ர் மேலாண்மை')}</h2>
            ...
            <div className="relative max-w-sm">
              ...
              <input
                ...
                placeholder={l('Search by name or email...', 'பெயர் ?…ல்லது மி?©்?©ஞ்சலால் தேடு?•...')}
                value={userSearch}
            ...
            <div className="bg-white rounded-2xl border border-borderLight shadow-sm overflow-hidden">
              {usersLoading ? (
                <div className="p-10 text-center text-[13px] font-bold text-[#6B7280]">{l('Loading users...', 'பய?©ர்?•ள் ஏற்று?•ிறது...')}</div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-[14px]">
                    <thead>
                      <tr className="bg-[#FAFAFA] border-b border-borderLight uppercase tracking-wider text-[11px] text-[#6B7280]">
                        ...
                        <th className="text-left px-6 py-4 font-black">{l('Email', 'மி?©்?©ஞ்சல்')}</th>
                        ...
                        <th className="text-center px-6 py-4 font-black">{l('Role', 'பங்?•ு')}</th>
                        <th className="text-center px-6 py-4 font-black">{l('Action', 'நடவடி?•்?•ை')}</th>
                      </tr>
                    </thead>
                          ...
                          <tr key={u.id} className="hover:bg-[#FAFAFA] transition-colors">
                            ...
                            <td className="px-6 py-4 text-center">
                              <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-black uppercase tracking-wider ${
                                ...
                                {u.role === 'admin' ? l('Admin', 'நிர்வா?•ி') : l('Customer', 'வாடி?•்?•ையாளர்')}
                              </span>
                            </td>
                            <td className="px-6 py-4 text-center">
                              {u.id === user?.id ? (
                                <span className="text-[12px] text-[#6B7280] font-bold uppercase tracking-wider">{l('You', 'நீங்?•ள்')}</span>
                              ) : (
                                <button
                                ...
                                >
                                  {u.role === 'admin' ? <><ShieldOff size={12} /> Remove Admin</> : <><ShieldCheck size={12} /> {l('Make Admin', 'நிர்வா?•ி ஆ?•்?•ு')}</>}
                                </button>
                              )}
                            </td>
                          </tr>
                  ...
                  {allUsers.length === 0 && !usersLoading && (
                    <p className="p-10 text-center text-[14px] font-bold text-[#6B7280] bg-[#FAFAFA]">{l('No users found.', 'பய?©ர் இல்லை.')}</p>
                  )}
                </div>
              )}
            </div>
            ...
            <p className="text-[12px] text-[#6B7280] font-bold">
              - {l('Role changes take effect upon next login.', 'பங்?•ு மாற்றம் ?…டுத்த முறை உள்நுழைந்தால் நடைமுறை?•்?•ு வரும்.')}
            </p>
          </div>
        )}
        </div>
      ...
      {invoicePreviewOrder && (() => {
        ...
        return (
          ...
          >
            <div className="flex max-h-[95vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-[#fbf8f2] shadow-2xl">
              <div className="flex shrink-0 items-center justify-between border-b border-[#e4d6c5]/60 bg-white px-4 py-3 sm:px-6">
                <div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="inline-flex min-h-[40px] items-center gap-1.5 rounded-xl border border-[#e4d6c5]/70 px-3 text-xs font-black text-[#111111] hover:bg-[#fbf8f2]"
                  >
                  ...
                  <button
                    ...
                    className="inline-flex h-10 w-10 items-center justify-center rounded-xl text-[#6B7280] hover:bg-[#fbf8f2] hover:text-[#111111]"
