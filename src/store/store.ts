...
export interface Product {
  ...
  productType?: 'standard' | 'lens' | 'other'
  brandType?: 'none' | 'non_branded' | 'branded'
  lensTypeId?: string | number | null
...
const toAuthUser = (profile: unknown, fallback?: SessionFallback): AuthUser => {
  ...
  return {
    ...
    email,
    mobile: normalizePhone(String(profileRow.mobile_number || profileRow.mobile || fallbackMeta.mobile || fallback?.phone || '')) || '',
...
const mapDbProduct = (input: unknown, categoriesById: Record<string, string> = {}): Product => {
  ...
  return {
    ...
    pricingType: p.pricing_type === 'manual' ? 'manual' : 'fixed',
    productType: p.product_type === 'lens' || p.product_type === 'other' ? p.product_type : 'standard',
    brandType: p.brand_type === 'branded' || p.brand_type === 'non_branded' ? p.brand_type : 'none',
    lensTypeId: typeof p.lens_type_id === 'string' || typeof p.lens_type_id === 'number' ? p.lens_type_id : null,
...
export const useFavStore = create<FavState>()(
  persist(
    ...
    { name: 'generic-business-favorites' },
  ),
)
