...
const BUSINESS_CURRENCY = new Intl.NumberFormat(businessConfig.locale, {
  style: 'currency',
...
export const formatInvoiceNo = (invNo: unknown): string => {
  ...
  // Preserve INV-YYYYMMDD-XXXXXX format as-is
...
export const formatCurrency = (value: number) => BUSINESS_CURRENCY.format(roundTo(value, 2)).replace(/\u00a0/g, ' ')
...
export const buildStructuredOrderItem = (input: {
  ...
  return {
    ...
    category_name: input.categoryName ? String(input.categoryName) : null,
