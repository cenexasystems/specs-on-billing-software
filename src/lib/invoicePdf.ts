...
function createInvoicePdfWithLogo(data: InvoicePdfData, logoDataUrl: string): Blob {
  const formattedNo = formatInvoiceNo(data.invoiceNo)
  const doc = new jsPDF({ unit: 'mm', format: 'a4' })
  const left = 16
  const right = 194
  const red = DOCUMENT_COLORS.primary
  const ink = DOCUMENT_COLORS.ink
  const muted = DOCUMENT_COLORS.muted
  let y = drawBrandHeader(doc, logoDataUrl, {
    title: 'Tax Invoice', documentNo: `#${formattedNo}`,
    date: new Date(data.date).toLocaleDateString('en-IN'), payment: data.paymentMode || 'POS',
  })

  const customerName = String(data.customerName || 'Walk-in Customer').trim()
  const customerPhone = formatPhoneDisplay(String(data.phone || '')) || '—'
  ...
  if ((data.discountAmount || 0) > 0) rows.push([`Coupon${data.couponCode ? ` (${data.couponCode})` : ''}`, `-${money(data.discountAmount || 0)}`, '#b45309'])
  if ((data.manualDiscountAmount || 0) > 0) rows.push(['Discount', `-${money(data.manualDiscountAmount || 0)}`, '#b45309'])
  ...
  if (data.advancePaid !== undefined) rows.push(['Advance paid', money(data.advancePaid), ink])
  if (data.finalPayment !== undefined) rows.push(['Final payment', money(data.finalPayment), ink])
  doc.setFontSize(9)
...
/** Regular downloadable invoice using the same local logo as the application. */
export async function invoicePdfFileWithCanonicalLogo(data: InvoicePdfData): Promise<File> {
  const logo = await loadCanonicalDocumentLogo()
  return new File([createInvoicePdfWithLogo(data, logo)], `Invoice-${formatInvoiceNo(data.invoiceNo)}.pdf`, { type: 'application/pdf' })
}
